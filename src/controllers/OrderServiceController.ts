...
export class OrderServiceController {
  ...
  public async create(req: Request, res: Response) {
    ...
    // Transforma a string "20/04/2025" em um objeto Date
    const [day, month, year] = opening_date.split("/").map(Number);
    const openingDateObj = new Date(year, month - 1, day); // month - 1 porque o mês começa do 0

    // Soma os dias do limite
    const daysToAdd = finalSoType?.days_limit || 0;
    const futureDate = new Date(openingDateObj);
    futureDate.setDate(openingDateObj.getDate() + daysToAdd);
    ...
    const orderService = await prisma.serviceOrder.create({
      data: {
        service_value: finalSoType?.service_value
          ? (finalSoType.service_value as number)
          : (service_value as number),
        displacement_value: finalSoType?.service_value
          ? (finalCity?.displacement_value as number)
          : (displacement_value as number),
        city: cityId,
        order_type: soTypeId,
        status: "LAUNCHED",
        photos_status: "Pendente",
        order_number,
        client_name,
        cep,
        company,
